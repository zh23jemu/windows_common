...
$log = get-content "c:\logs\$(get-date -f yyyy-MM-dd)-WindowsUpdate.log" | out-string

$params = @{ 
    #InlineAttachments = $images
    #Attachments = "c:\logs\$(get-date -f yyyy-MM-dd)-WindowsUpdate.log"
    Body = $log
    BodyAsHtml = $false 
    Subject = 'Windows Update Results (' + $env:COMPUTERNAME + ') ' + (get-date -Format 'yyyyMMdd')
    From = 'report <report@canadiansolar.com>' 
    To = 'v_shikai.qiu@canadiansolar.com'
    Cc = 'billy.zhou@canadiansolar.com'
    #Bcc = 'billy.zhou@canadiansolar.com'
